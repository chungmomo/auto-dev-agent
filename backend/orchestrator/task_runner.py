...
real Git/GitHub integration is SDD §12 step 5, not step 2. This lets the full
CREATED -> ... -> DONE lifecycle be exercised end-to-end with mock agents, per the
"test luồng" intent of SDD §12 step 2.
"""
...
from backend.models.task import StepStatus, Task, TaskEvent, TaskStatus
...
class TaskRunner:
    ...
    def run(self, task: Task) -> Task:
        ...
        for step in task.steps:
            ...
            step.generated_code = self.codegen_agent.generate(step)
        ...
        if not result.passed:
            ...
            self._emit(task, "error", extra={"reason": "tests failed"})
            return task
