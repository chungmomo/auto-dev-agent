...
def make_runner(passed: bool) -> TaskRunner:
    return TaskRunner(
        planning_agent=MockPlanningAgent(),
        codegen_agent=MockCodeGenAgent(),
        test_runner=MockTestRunner(passed=passed),
    )


def test_full_run_reaches_done_on_passing_tests():
    task = Task(user_prompt="Add JWT login API")
    runner = make_runner(passed=True)
...
def test_run_stops_at_needs_attention_when_tests_fail():
    task = Task(user_prompt="Add JWT login API")
    runner = make_runner(passed=False)
